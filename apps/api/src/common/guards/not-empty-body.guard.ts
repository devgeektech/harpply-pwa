...
export class NotEmptyBodyGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const allowEmpty = this.reflector.getAllAndOverride<boolean>(ALLOW_EMPTY_BODY, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    if (method !== 'PATCH' && method !== 'PUT' && method !== 'POST') return true;
    ...
    if (Object.keys(body).length === 0) {
      throw new BadRequestException('Request body cannot be empty. Provide at least one field to update.');
    }
    return true;
  }
}
